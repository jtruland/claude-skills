...
| [`build-recipe-format`](build-recipe-format/) | Write or restructure a **step-by-step build recipe** for something assembled by hand in a GUI (a Power Automate flow, a canvas screen, a list schema). Lead with a structure tree, collapse N-way repeats into a table of literal values, give every expression its own complete pasteable block, and state a value for every required field — plus why fill-in-the-blank templates are the wrong trade in a document people paste from, and a script to verify the expansions you publish instead. |
| [`unication-codeplug`](unication-codeplug/) | Read and edit **Unication GxPPS `.unipps` codeplugs** (G-series P25 pagers) outside the CPS, and write files the CPS still loads — bulk talkgroup edits, dial positions, zones, per-zone site restriction. Documents the container (60-byte header, two CRC-16/CCITT fields, 32-bit length, a payload zip of SQLite **2.1** databases that `sqlite3` cannot open) and the traps: a timestamp coupled to a header CRC, a site join on the wrong key, `VACUUM` after filtering, and a WACN that passes every checksum while making the pager look dead. Ships `unipps.py`. |
